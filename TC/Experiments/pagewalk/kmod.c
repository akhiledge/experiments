...
long myIoctl(struct file *f,unsigned int num, unsigned long num1){
    ...
    printk("num1 %lx\n",num1);   
    unsigned long *pt = (unsigned long *)((((myTask->mm->pgd))+(num1>>22)));
    printk ("pt %.8lx\n",pt);
    unsigned long *pt1 = kmap(pfn_to_page(*pt));
//    printk ("*pt %.8lx",pt);
    unsigned long *p = (unsigned long *)(pt1 + ((num >> 12)&0x00000fff));
    printk ("p %.8lx",p);
    unsigned long *p1 = kmap(pfn_to_page(*p));
    printk ("p1 %.8lx",p1);

//    printk ("t %.8lx",((num1<<10)>>22));
